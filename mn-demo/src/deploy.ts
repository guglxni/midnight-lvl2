...
// witness reads an off-chain secret; deploy only runs the constructor, which
// does not call the witness. The witness implementation is still required
// because the compiled contract checks that `userSecret` is a function.
...
// Placeholder witness for deployment. The constructor does not call it, and
// this value is not a wallet seed. A real caller supplies their own secret
// when invoking `increment`.
const counterWitnesses = {
  userSecret: (context: { privateState: unknown }) => {
...
async function main() {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // initialPrivateState. The counter constructor takes no arguments and
      // does not read the witness. (Statically-typed contracts can omit args
      // entirely; this script loads the contract dynamically, so the
